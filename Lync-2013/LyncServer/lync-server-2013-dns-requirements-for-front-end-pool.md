...
Antes de operar la topología después de haberla implementado, asegúrese de crear los siguientes registros de Active Directory y DNS (según las necesidades de características específicas que determinen):
  ...
  - Existe un registro SRV de DNS para cada dominio SIP si piensa usar el inicio de sesión automático para clientes en forma de \_ sipinternaltls \_ TCP. \<SIP domain\> . Si va a usar la configuración manual para clientes, este registro no es necesario.
