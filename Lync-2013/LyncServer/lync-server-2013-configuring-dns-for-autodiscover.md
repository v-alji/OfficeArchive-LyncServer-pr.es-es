...
Los registros DNS pueden ser A (host) o registros CNAME, en función de su capacidad para crear certificados nuevos con el nombre alternativo de asunto adicional (SAN). Si no puede solicitar e implementar un nuevo certificado externo (público) con el lyncdiscover.\<domain name\> SAN, use el procedimiento para usar el puerto HTTP/TCP 80. Los procedimientos siguientes describen cómo crear registros DNS internos y externos.
